...
func_hangul_install() {
	...
	sudo sed -i 's/IF_CNOFIG_DEFAULT_MODE=auto/IF_CNOFIG_DEFAULT_MODE=fcitx/g' /etc/default/im-config
	sudo sed -i 's/XKBDLAYOUT="us"/XKBDLAYOUT="en"/g' /etc/default/keyboard
}
func_hangul_install
...
func_serial_set() {
	...
	sudo sed -i 's/console=serial0,115200/d' /boot/cmdline.txt
}
func_serial_set
